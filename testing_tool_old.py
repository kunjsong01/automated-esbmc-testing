...
# old ESBMC version show goto only
OLD_GOTO = False
# new ESBMC version show goto only
NEW_GOTO = False
...
class CTestCase(BaseTest):
    ...
    def _initialize_test_case(self):
        with open(os.path.join(self.test_dir, "test.desc")) as fp:
            ...
            if CTEST_CPP_INCLUDE is not None:
                self.test_args += " -I "
                self.test_args += CTEST_CPP_INCLUDE
                #print("\n This is self.test_args\n");
                #print(self.test_args)
                #print("\n This is self.test_args type\n");
                #print(type(self.test_args))

            global OLD_GOTO
            if(OLD_GOTO):
                self.test_args += " --show-goto-functions "
...
class XMLTestCase(BaseTest):
    ...
    def _initialize_test_case(self):
        ...
        #print("\nThis is root[4].text\n");
        #print("\n{}\n", root[4].text)
        if root[4].text is None:
            self.test_args: str = "--unwind 10"
        else:
    ...
    def generate_run_argument_list(self, *tool):
        ...
        if XMLTestCase.CPP_INCLUDE_DIR is not None:
            #print("Got non include dir\n");
            #print("\n@@ This is result before append: \n")
            #print(result)
            result.append('-I');
            result.append(XMLTestCase.CPP_INCLUDE_DIR)
            #print("\n@@ This is result after append: \n")
            #print(result)

        global OLD_GOTO
        if(OLD_GOTO):
            result.append('--show-goto-functions')
...
def _arg_parsing():
    ...
    parser.add_argument("--noembeddedcpp", default=False, action="store_true",
            help="Flag to specify not using embedded cpp include")
    parser.add_argument("--oldgoto", default=False, action="store_true",
    ...
    global OLD_GOTO
    if(main_args.oldgoto):
        OLD_GOTO = True

    global NEW_GOTO
    if(main_args.newgoto):
        NEW_GOTO = True
